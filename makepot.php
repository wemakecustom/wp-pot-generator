...
class makepot
{
    ...
    public $ms_files = array( 'ms-.*', '.*/ms-.*', '.*/my-.*', 'wp-activate\.php', 'wp-signup\.php', 'wp-admin/network\.php', 'wp-admin/includes/ms\.php', 'wp-admin/network/.*\.php', 'wp-admin/includes/class-wp-ms.*' );
    ...
    public function xgettext($project, $dir, $output_file, $placeholders = array(), $excludes = array(), $includes = array())
    {
        ...
        $originals = $this->extractor->extract_from_directory( $dir, $excludes, $includes, $this->get_relative_path($output_file, $dir) );
        $pot = new PO;
